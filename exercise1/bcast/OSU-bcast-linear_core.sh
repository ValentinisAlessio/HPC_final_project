...
#SBATCH --output=bcast-linear-core.out
#SBATCH --error=bcast-linear-core.err
#SBATCH --nodes=2
#SBATCH --ntasks=256
#SBATCH -p EPYC
#SBATCH --exclusive
...
csv_file="../results_def/bcast_linear_results_wu_core.csv"
...
for np in {1..256}; do
    ...
    tail -n 12 | awk -v np="$np" '{printf "linear,core,%s,%s,%s\n",np,$1,$2}' | sed 's/,$//' >> $csv_file
done
