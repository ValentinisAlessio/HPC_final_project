...
#SBATCH --output=bcast-bintree-socket.out
#SBATCH --error=bcast-bintree-socket.err
#SBATCH --nodes=2
#SBATCH --ntasks=256
#SBATCH -p EPYC
#SBATCH --exclusive
...
csv_file="../results_def/bcast_bintree_results_wu_socket.csv"
...
for np in {193..256}; do
    ...
    mpirun -np $np --map-by socket --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 5 $src_path/osu_bcast -m 2048 -x 100 -i 10000 |\
    # Append results to CSV file
    tail -n 12 | awk -v np="$np" '{printf "bin_tree,socket,%s,%s,%s\n",np,$1,$2}' | sed 's/,$//' >> $csv_file
done
