...
#SBATCH --output=bcast-def-socket.out
#SBATCH --error=bcast-def-socket.err
#SBATCH --nodes=2
#SBATCH --ntasks=256
#SBATCH -p EPYC
#SBATCH --exclusive
...
# for np in {1..256}; do
for np in {143..256}; do
    echo "Running MPI Bcast benchmark: map=$mapping, np=$np, broadcast_algo=default ..."
    # Run MPI Bcast benchmark and capture output
    mpirun -np $np --map-by socket --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 0 $src_path/osu_bcast -m 2048 -x 100 -i 10000 |\
    # Append results to CSV file
    tail -n 12 | awk -v np="$np" '{printf "default,socket,%s,%s,%s\n",np,$1,$2}' | sed 's/,$//' >> $csv_file
done
