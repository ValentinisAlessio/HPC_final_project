...
csv_file="../results_def/barrier_ring_results_wu_core.csv"
...
for np in {1..256}; do
    ...
    mpirun -np $np --map-by core --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_barrier_algorithm 2 $src_path/osu_barrier -x 100 -i 10000 |\
    # Append results to CSV file
    tail -n 1 | awk -v np="$np" '{printf "ring,core,%s,%s,%s\n",np,$1,$2}' | sed 's/,$//' >> $csv_file
done
# done
