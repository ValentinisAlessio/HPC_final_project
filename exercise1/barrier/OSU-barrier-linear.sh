...
#SBATCH --output=barrier-linear.out
#SBATCH --error=barrier-linear.err
#SBATCH --nodes=2
#SBATCH --ntasks=256
#SBATCH -p EPYC
#SBATCH --exclusive
...
csv_file="../results_def/barrier_linear_results_wu.csv"
...
for mapping in $map_values; do
    for np in {1..256}; do
        ...
        mpirun -np $np --map-by $mapping --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_barrier_algorithm 1 $src_path/osu_barrier -x 100 -i 10000 |\
        # Append results to CSV file
        tail -n 1 | awk -v mapping="$mapping" -v np="$np" '{printf "linear,%s,%s,%s,%s\n",mapping,np,$1,$2}' | sed 's/,$//' >> $csv_file
    done
done
