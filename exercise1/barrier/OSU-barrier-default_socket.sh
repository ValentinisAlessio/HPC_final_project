...
#SBATCH --output=barrier-default-socket.out
#SBATCH --error=barrier-default-socket.err
#SBATCH --nodes=2
#SBATCH --ntasks=256
#SBATCH -p EPYC
#SBATCH --exclusive
...
csv_file="../results_def/barrier_default_results_wu_socket.csv"
...
for np in {1..256}; do
    ...
    mpirun -np $np --map-by socket --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_barrier_algorithm 0 $src_path/osu_barrier -x 100 -i 10000 |\
    # Append results to CSV file
    tail -n 1 | awk -v np="$np" '{printf "default,socket,%s,%s,%s\n",np,$1,$2}' | sed 's/,$//' >> $csv_file
done
# done
