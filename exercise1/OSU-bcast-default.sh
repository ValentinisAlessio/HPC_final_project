...
for mapping in $map_values; do
    ...
    for np in $np_values; do
        ...
        mpirun -np $np --map-by $mapping --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 0 $src_path/osu_bcast -i 100 -x 10000 |\
        # Append results to CSV file
        tail -n 21 | awk -v mapping="$mapping" -v np="$np" '{printf "default,%s,%s,%s,%s\n",mapping,np,$1,$2}' | sed 's/,$//' >> $csv_file
    done
done
