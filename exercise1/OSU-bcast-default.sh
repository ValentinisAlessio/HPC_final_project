...
#SBATCH --output=OSU-def-bcast.out
#SBATCH --error=OSU-def-bcast.err
#SBATCH --nodes=2
#SBATCH --ntasks=256
#SBATCH -p EPYC
#SBATCH --exclusive
...
csv_file="results/bcast_def_results.csv"
...
for mapping in $map_values; do
    ...
    for np in $np_values; do
        ...
        output=$(mpirun -np $np --map-by $mapping --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 0 $src_path/osu_bcast)
        # Append results to CSV file
        tail -n 21 "$output" | awk -v mapping="$mapping" -v np="$np" '{printf "default,%s,%s,%s,%s\n",mapping,np,$1,$2}'\
        | sed 's/,$//' >> $csv_file
    done
done
