...
#SBATCH --output=OSU-linear-bcast-thin.out
#SBATCH --error=OSU--linear-bcast-thin.err
#SBATCH --nodes=2
#SBATCH --ntasks=72
#SBATCH -p THIN
#SBATCH --exclusive
...
csv_file="results/bcast_linear_results.csv"
...
for mapping in $map_values; do
    ...
    for np in {1...72}; do
        ...
        output=$(mpirun -np $np --map-by $mapping --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 1 $src_path/osu_bcast -x 1000 -i 10000)
        # Append results to CSV file
        tail -n 21 "$output" | awk -v mapping="$mapping" -v np="$np" '{printf "linear,%s,%s,%s,%s\n",mapping,np,$1,$2}'\
        | sed 's/,$//' >> $csv_file
    done
done
